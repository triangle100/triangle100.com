<script>
	import { DateTime } from "luxon";

	export let project;

	function openLink() {
		window.open(project?.link, "_blank");
	}
</script>

<button class="m-2" on:click={openLink}>
	<div
		class="flex h-full cursor-pointer flex-col overflow-hidden rounded-lg bg-bluegray-700 transition-transform hover:scale-105"
	>
		<div
			class="relative bg-bluegray-900 pt-[50%] text-6xl font-bold leading-[0px] text-bluegray-700"
		>
			<div
				class="absolute top-1/2 left-1/2 w-full -translate-x-1/2 -translate-y-1/2"
			>
				{#if project.thumb != null}
					<img src={project.thumb} alt="sample" />
				{:else}
					<span class="pointer-events-none select-none">?</span>
				{/if}
			</div>
		</div>
		<div class="flex flex-col gap-2 bg-bluegray-700 p-3 text-left">
			<div class="flex flex-col">
				<span class="text-sm font-bold">{project.title}</span>
				<span class="dim text-xs">
					Last updated: {DateTime.fromMillis(
						project.updatedAt
					).toFormat("DD t")}
				</span>
			</div>
			<span class="text-sm">{project.desc}</span>
		</div>
	</div>
</button>
